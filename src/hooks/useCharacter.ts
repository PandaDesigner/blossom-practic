...
export const useCharacter = () => {
    ...
    const availableSpecies = useMemo(() => {
        ...
        const species = [...new Set(allCharacters.results.map((char: { species: unknown; }) => char.species))];
        return species.sort();
    }, [allCharacters]);
    ...
    const availableStatuses = useMemo(() => {
        if (!allCharacters?.results) return [];
        const statuses = [...new Set(allCharacters.results.map((char: { status: unknown; }) => char.status))];
    ...
    const filteredCharacters = useMemo(() => {
        ...
        return characters.filter((char: { id: string; species: string; status: string; }) => {
